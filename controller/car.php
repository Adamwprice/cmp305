...
try {
  $writeDB = DB::connectWriteDB();  //Connects to the database given file/
  $readDB = DB::connectReadDB();  //Reads the database
}
catch(PDOException $exception) {  //Catches errors that appear if the database fails to connect
...
if(array_key_exists("carid", $_GET)) {  // Obtains database ID for car
  $carid = $_GET['carid'];

  if($carid == '' || !is_numeric($carid)) { //Contacts database to check if ID is in the database
    $response = new Response();
  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET') {  //Response with the database record with given ID in the database
    ...
    try {
      ...
      while($row = $query->fetch(PDO::FETCH_ASSOC)) { //Fetches database data of specific row
    ...
    catch(PDOException $exception) {  //catches errors if function fails to obtain from database
  ...
  elseif($_SERVER['REQUEST_METHOD'] === 'DELETE')  {  //Function send to delete records from a given ID
...
// END OF DELETE
  elseif ($_SERVER['REQUEST_METHOD'] === 'PATCH') { //Fetches requested ID to allow for update
...
elseif(array_key_exists("carmake", $_GET)) {  //Searches the database for records with car make
  ...
  if($_SERVER['REQUEST_METHOD'] === 'GET') {  //this receives all  database records from a specificed car make
    ...
    catch(PDOException $exception) {  // Error that shows if there is a problem when finding a database record
...
elseif(empty($_GET)) {
  ...
  if ($_SERVER['REQUEST_METHOD'] === 'POST') { // creates new database entry
try{
    if($_SERVER['CONTENT_TYPE'] !== 'application/json') {
      $response = new Response();
      $response->setHttpStatusCode(400);
    ...
    $query = $writeDB->prepare('insert into tbl_cars (make, model, description, colour, registration, price) values (:make, :model, :description, :colour, :registration, :price)'); //inserts the data into the database
